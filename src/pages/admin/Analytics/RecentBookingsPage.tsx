          ...
          <div className="d-flex align-items-center justify-content-between mb-5">
            <div className="d-flex align-items-center gap-4">
              ...
              <div>
                ...
                <p className="text-muted mb-0">Real-time admin view across all turfs</p>
              </div>
            </div>
            ...
            <div className="card-body p-0">
              {loading ? (
                <div className="text-center py-5">
                  ...
                  <p className="mt-4 text-muted fs-5">Loading latest bookings...</p>
                </div>
              ) : (
                <>
                  <div className="table-responsive">
                    <table className="table table-hover align-middle mb-0">
                      <thead className="bg-gradient text-white" style={{ background: "linear-gradient(135deg, #1e40af, #3b82f6)" }}>
                        <tr>
                          ...
                          <th className="text-end py-4 fw-semibold pe-4">Paid (₹)</th>
                          <th className="text-end py-4 fw-semibold pe-4">Pending (₹)</th>
                          <th className="text-center py-4 fw-semibold pe-4">Status</th>
                        </tr>
                      </thead>
                      <tbody className="bg-white">
                        {pageData.map((b, i) => (
                          <tr key={`${b.booking_id}-${i}`} className="border-bottom">
                            <td className="ps-4 py-4 fw-medium text-primary" style={{ fontFamily: "monospace", fontSize: "0.92rem" }}>
                              {b.booking_id}
                            </td>
                            <td className="py-4 fw-medium">{b.booking_username || "Guest"}</td>
                            <td className="py-4 fw-semibold text-dark">{b.turf_name || b.turfName}</td>
                            <td className="py-4 text-muted">{b.sport} / {b.court}</td>
                            <td className="py-4">{b.date}</td>
                            <td className="py-4 fw-medium">{b.slot_start_time || b.time}</td>
                  ...
                  {filtered.length === 0 && (
                    <div className="text-center py-5">
                      <div className="text-muted fs-4 mb-3">No bookings found</div>
                  ...
                  <div className="d-flex justify-content-between align-items-center px-4 py-4 bg-light border-top">
                    <div className="text-muted fw-medium">
                      Showing <strong>{pageData.length}</strong> of <strong>{filtered.length}</strong> bookings
                    </div>
